...
exec(command, (error, stdout) => {
  ...
  output.forEach(entry => {
    if (!vulnerabilities[entry[SEVERITY_COLUMN]]) return;
  ...
  const warningMessage =
    generateVulnerabilityMessage(vulnerabilities, "low") +
    generateVulnerabilityMessage(vulnerabilities, "moderate");
  if (warningMessage)
    console.log(
      `##vso[task.logissue type=warning;]NPM Package Vulnerabilities:${warningMessage}`
    );

  const errorMessage =
    generateVulnerabilityMessage(vulnerabilities, "high") +
    generateVulnerabilityMessage(vulnerabilities, "critical");
  if (errorMessage)
    console.log(
      `##vso[task.logissue type=error;]NPM Package Vulnerabilities:${errorMessage}`
    );
});
